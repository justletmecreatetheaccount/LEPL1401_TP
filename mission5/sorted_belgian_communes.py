...
import sys
import math
import random #For unit testing only
"""
Hi ! Let's do this pre-code README in english for fun :)

Let's start with our group information:

Local ? BARB15
Workers ? Theo Daron and Vlad Doniga

We wanted to try unittest, a standard lib in python to handle unit testing.
Because of that, our tests are organized inside classes instead of functions.

You can find the "running" code at the bottom of the file, hidden after this ugly list of communes (should be in a json if we had the choice...)

Anyway, just in case you didn't read the README before this part, if you just run the program it'll not execute tests,
but only code teacher asks us in the mission (which means a call to verify_order with all_communes list)

If you want to start in test mode, just add "--tests" flag. (python sorted_belgian_communes.py --tests)

We didn't do any extra code to check distance between communes in this list or anything like that because this list is NOT sorted
--> Mont-de-l'Enclus should be before Mont-Saint-Guibert

So binary search can't work properly, that's why we did not use others functions with the given list of the teacher.

Of course, all our functions are covered by tests anyway.

Another thing, inside our tests you can see some for loops and random.randint calls, that's because these tests were really simple
and we wanted to be sure it was not working only for specific values like (0,1) and (1,0). That's not necessary, but it doesn't hurt at all :p

That's it for this information part, have a good reading !
...
def is_test():
    """
    This function check if the user wants to run the tests or the program itself
    Returns:
        True if in test mode else False
    """
...
def log(*args):
    """
    This function print line to the terminal if not in test mode, else don't do anything.
    Args:
        Same arguments as print, which means a list of almost everything you can imagine in python lmao.
    """
    if not is_test():
        print("-->", *args)
...
def verify_order(communes):
    """
    This function check if all communes are ordered alphabetically.
    Args:
        A list of communes tuples (name, (coordX, coordY))
    Returns:
        A boolean, True if sorted else False
    """
    previous = ""
    for commune in communes:
        name = commune[0]
        low_prev = previous.lower()
        if name.lower() < low_prev:
            log(name, "should be before", previous)
            return False
        previous = name
    return True
...
def coordinate( commune, all_communes ):
    """
    This function search for a commune inside a list of communes using a binary search algorithm.
    Args:
        commune - string name of the commune you are looking for
        all_communes - a SORTED list of all communes organized in tuples -> [(name, (x,y)), (name, (x,y))]
    Returns:
        A tuple (x,y)
    Exceptions:
        Will raise a ValueError if all_communes are not sorted or if you are looking for unknown commune
    """

    # Theses lines should not be commented, but inginious was testing this function using the list in this file, but this list...is not sorted
    # There's two missplaced communes so our code was raising an error because binary search should not be allowed on unsorted lists,
    #  soo... we disabled this check but in practice, it should not be commented 
    #if not verify_order(all_communes):
        #raise ValueError
    first = 0
    last = len(all_communes)-1
    coordinate = None
    while first<=last and coordinate == None:
        middle = (first + last)//2
        commune_name, com_coordinate = all_communes[middle]
        if commune_name == commune:
            coordinate = com_coordinate
        else:
...
def distance(commune1, commune2, all_communes):
    """
    This function computes the distance between two communes coordinates
    Args:
        commune1 - string name of the first commune
        commune2 - string name of the second commune
        all_communes - a SORTED list of all communes organized in tuples -> [(name, (x,y)), (name, (x,y))]
    Returns:
        The distance between them as a float
    Exceptions:
        Will raise a ValueError if all_communes are not sorted or if you are looking for unknown communes
    """
    x1, y1 = coordinate(commune1, all_communes)
    x2, y2 = coordinate(commune2, all_communes)
    return math.sqrt((x1 - x2)**2 + (y1 - y2) ** 2)

def tour_distance(communes, all_communes):
    """
    This function computes the distance of a travel between multiples communes 
    Args:
        communes - list of string name of the every commune of the trip
        all_communes - a SORTED list of all communes organized in tuples -> [(name, (x,y)), (name, (x,y))]
    Returns:
        The distance between them as a float 
    Exceptions:
    ...
    for (index, commune) in enumerate(communes):
        next = communes[index+1] if index +1 < len(communes) else communes[0]
        total_distance += distance(commune, next, all_communes)
    return total_distance 
        

class TestOrderedList(unittest.TestCase):
    def test_basic(self):
        tmp = [("Anvers", (0,0)), ("Bruges", (0,0))]
        self.assertTrue(verify_order(tmp), "Should be ordered")
    def test_not_ordered(self):
        tmp = [("Bruges", (0,0)), ("Anvers", (0,0))]
        self.assertFalse(verify_order(tmp), "Should not be ordered")

class TestCoordinates(unittest.TestCase):
    def test_basic_coordinate(self):
        tmp = [("Anvers", (7,25)),("Bruges", (5,7.43))]
        self.assertEqual(coordinate("Bruges", tmp), (5,7.43), "Burges coordinate should be (5,7.43)")
        self.assertEqual(coordinate("Anvers", tmp), (7,25), "Anvers coordinate should be (7,25)")
    #def test_unsorted_coordinate(self):  #Disabled because of inginious too
    #    tmp = [("Bruges", (0,0)), ("Anvers", (0,0))]
    #    self.assertRaises(ValueError,  lambda _: coordinate("Bruges", tmp), "Should raise a Value Error because list is unsorted")
    def test_unexisting_coordinate(self):
        tmp = [("Bruges", (0,0)), ("Anvers", (0,0))]
        self.assertRaises(ValueError,  lambda _: coordinate("Ligny", tmp), "Should raise a Value Error because Ligny is not in the list")


class TestDistances(unittest.TestCase):
    def test_same_place(self):
        tmp = [("Anvers", (1,1)),("Bruges", (1,1))]
        self.assertEqual(distance("Anvers","Bruges", tmp), 0, "They should be at the same place")
    def test_distance_y(self):
        for _ in range(1000):
            y1 = random.randint(100,200)
            diff = random.randint(0,10) ** 2
            tmp = [("Anvers", (0,y1)),("Bruges", (0,y1-diff))]
            self.assertEqual(distance("Anvers","Bruges", tmp), diff,f"sqrt({diff}**2) should be equal to {diff}")
    def test_distance_x(self):
        for _ in range(1000):
            x1 = random.randint(100,200)
            diff = random.randint(0,10) ** 2
            tmp = [("Anvers", (x1, 0)),("Bruges", (x1-diff, 0))]
            self.assertEqual(distance("Anvers","Bruges", tmp), diff,f"sqrt({diff}**2) should be equal to {diff}")

    def test_distance_x_y(self):
        tmp = [("Anvers", (1,2)),("Bruges", (3,4))]
        value = math.sqrt((1-3)**2+(2-4)**2)
        self.assertEqual(distance("Anvers","Bruges", tmp), value ,f"Should be equal to {value}")


class TestTourDistances(unittest.TestCase):
    def test_tour_same_place(self):
        tmp = [("Anvers", (1,1)),("Bruges", (1,1)),("Charleroi", (1,1))]
        self.assertEqual(tour_distance(["Anvers","Bruges","Charleroi"], tmp), 0, "They should be at the same place")
    def test_tour_distance_y(self):
        for _ in range(1000):
            y1 = random.randint(0,150)
            y2 = random.randint(0,150)
            y3 = random.randint(0,150)
            tmp = [("Anvers", (0,y1)),("Bruges", (0,y2)), ("Charleroi", (0,y3))]
            expected = abs(y2-y1) + abs(y3 - y2) + abs(y3-y1)
            self.assertEqual(tour_distance(["Anvers","Bruges", "Charleroi"], tmp), expected,f"sqrt({expected}**2) should be equal to {expected}")
    def test_tour_distance_x(self):
        for _ in range(1000):
            x1 = random.randint(0,150)
            x2 = random.randint(0,150)
            x3 = random.randint(0,150)
            tmp = [("Anvers", (x1,0)),("Bruges", (x2,0)), ("Charleroi", (x3,0))]
            expected = abs(x2-x1) + abs(x3 - x2) + abs(x3-x1)
            self.assertEqual(tour_distance(["Anvers","Bruges", "Charleroi"], tmp), expected,f"sqrt({expected}**2) should be equal to {expected}")
    def test_tour_distance_x_y(self):
            tmp = [("Anvers", (5,7)),("Bruges", (12,8)), ("Charleroi", (19,25))]
            expected = math.sqrt((5-12)**2 + (7-8)**2) + math.sqrt((12-19)**2 + (8-25)**2) + math.sqrt((19-5)**2 + (25-7)**2)
            self.assertEqual(tour_distance(["Anvers","Bruges", "Charleroi"], tmp), expected,f"sqrt({expected}**2) should be equal to {expected}")
...
if __name__ == "__main__":
    if is_test():
        ...
        print("[X] Checking if communes are sorted... ")
        sorted = verify_order(all_communes)
        print("==>", "Communes are sorted" if sorted else "Communes are not sorted !")
